...
contract ITrustInsure
{
    ...
    function ifNotPaused() internal view{
        require(_paused == false, "Contract Frozen");
    }
    ...
    function initialize(
        ...
        INXMaster master = INXMaster(nxMasterAddress); 
        _nxmTokenAddress = master.tokenAddress();
        LOCKED = FALSE;
    }
    ...
    returns (uint256) {
        ...
        require(msg.value == _iTrustFeePercentage.mul(_getCoverPrice(coverData)).div(10000).add(_getCoverPrice(coverData))
                , "Eth Sent and Price Mismatch" );
         
        Exchange memory purchaseExchange = _exchanges[address(exchangeAddress)];
        require(
            purchaseExchange.treasuryAddress != address(0) && purchaseExchange.active == true,
    ...
    {        
        ...
        require(cover.iTrustOwned == true, "submit NFT");
        require(cover.claimsAllowed == true && _userOwnsCover(userGUID, coverId) ==TRUE);//check cover
        ...
        returns (uint) 
    {
        ...
        IERC20 nxmToken = IERC20(_nxmTokenAddress);
        return nxmToken.balanceOf(_distributorAddress);    
    }      
